...
module Spree
  module Admin
    class AdjustmentsController < ::Admin::ResourceController
      ...
      before_action :set_order_id, only: %i[create update]
      before_action :skip_changing_canceled_orders, only: %i[create update]
      after_action :update_order, only: %i[create update destroy]
      after_action :apply_tax, only: %i[create update]
