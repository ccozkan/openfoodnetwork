...
value: line_item.price)
  ...
  context "listing adjustments" do
    let!(:admin_adjustment) {
      create(
:adjustment, label: "Admin Adjustment", order: order, adjustable: order,
             amount: 12, originator: nil, state: "closed")
    }
    let!(:ineligible_adjustment) {
      create(
:adjustment, label: "Ineligible Adjustment", order: order, adjustable: order,
    ...
    let!(:included_tax_rate) {
      create(
:tax_rate, amount: 12, included_in_price: true, zone: zone,
           calculator: ::Calculator::DefaultTax.new)
    }
    let!(:additional_tax_rate) {
      create(
:tax_rate, amount: 34, included_in_price: false, zone: zone,
           calculator: ::Calculator::DefaultTax.new)
    }
    let!(:included_tax_adjustment) {
      create(
:adjustment, label: "Included Tax Adjustment", order: order,
    ...
    let!(:additional_tax_adjustment) {
      create(
:adjustment, label: "Additional Tax Adjustment", order: order, adjustable: order.shipment,
             amount: 78, originator: additional_tax_rate, state: "closed")
    }
    let!(:enterprise_fee) { create(:enterprise_fee) }
    let!(:line_item_enterprise_fee) {
      create(
:adjustment, label: "Line Item Fee", order: order, adjustable: order.line_items.first,
             amount: 91, originator: enterprise_fee, state: "closed")
    }
    let!(:order_enterprise_fee) {
      create(
:adjustment, label: "Order Fee", order: order, adjustable: order,
